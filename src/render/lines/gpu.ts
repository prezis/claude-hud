...
import type { GpuConsumer, GpuStatus, ClaudeSession } from "../../gpu.js";
...
function shortDeviceName(name: string): string {
  ...
  if (rtxMatch) return rtxMatch[1];
  const gtxMatch = name.match(/GTX\s*(\w+)/i);
  if (gtxMatch) return gtxMatch[1];
  // Strip vendor prefix.
...
interface GroupedConsumers {
  ollama: GpuConsumer[];
  llamaServer: GpuConsumer[];
  vllm: GpuConsumer[];
  claude: GpuConsumer[];
  bun: GpuConsumer[];
  browser: GpuConsumer[];
...
function groupConsumers(consumers: GpuConsumer[]): GroupedConsumers {
  const grouped: GroupedConsumers = {
    ollama: [], llamaServer: [], vllm: [], claude: [], bun: [], browser: [], other: [],
  };
  for (const c of consumers) {
    switch (c.kind) {
      case "ollama": grouped.ollama.push(c); break;
      case "llama-server": grouped.llamaServer.push(c); break;
      case "vllm": grouped.vllm.push(c); break;
      case "claude": grouped.claude.push(c); break;
      case "bun": grouped.bun.push(c); break;
      case "browser": grouped.browser.push(c); break;
      default: grouped.other.push(c); break;
    }
  }
  return grouped;
}

function renderHeaderLine(status: GpuStatus, compact: boolean): string {
...
function renderConsumerRow(line: string): string {
  return `  ${dim("↳")} ${line}`;
}

function describeOllama(c: GpuConsumer, status: GpuStatus, now: number): string {
...
function describeLlamaServer(c: GpuConsumer): string {
  const portPart = c.detail ? ` · ${dim(c.detail)}` : "";
  return `${dim("llama-server")} · ${formatGpuBytes(c.memoryBytes)}${portPart}`;
}

function describeVllm(c: GpuConsumer): string {
  return `${dim("vllm")} · ${formatGpuBytes(c.memoryBytes)} · ${dim(`pid ${c.pid}`)}`;
}

function describeClaude(c: GpuConsumer): string {
  const tmux = c.tmuxLabel ? ` ${dim(`(${c.tmuxLabel})`)}` : "";
  return `${dim("claude")}${tmux} · ${formatGpuBytes(c.memoryBytes)} · ${dim(`pid ${c.pid}`)}`;
}

function describeBun(c: GpuConsumer): string {
  const tmux = c.tmuxLabel ? ` ${dim(`(${c.tmuxLabel})`)}` : "";
  return `${dim("bun")}${tmux} · ${formatGpuBytes(c.memoryBytes)} · ${dim(`pid ${c.pid}`)} (no claude session)`;
}

function describeBrowser(c: GpuConsumer): string {
  const lower = c.processName.toLowerCase();
  let bin = "browser";
  if (lower.includes("playwright")) bin = "playwright";
  else if (lower.includes("chromium")) bin = "chromium";
  else if (lower.includes("chrome")) bin = "chrome";
  else if (lower.includes("electron")) bin = "electron";
  else if (lower.includes("firefox")) bin = "firefox";
  return `${dim(bin)} · ${formatGpuBytes(c.memoryBytes)} · ${dim(`pid ${c.pid}`)}`;
}

function describeOther(c: GpuConsumer): string {
...
function describeClaudeSession(session: ClaudeSession): string {
 ...
 */
export function formatGpuLines(status: GpuStatus | null, compact: boolean, now: number): string {
  if (!status) return "";

  const headerLine = renderHeaderLine(status, compact);
  if (compact) return headerLine;
  ...
  const hasAiConsumer =
    grouped.ollama.length + grouped.llamaServer.length + grouped.vllm.length > 0;
...
export function renderGpuLine(ctx: RenderContext, status: GpuStatus | null): string | null {
  const display = ctx.config?.display;
  if (display?.showGpu !== true) return null;
